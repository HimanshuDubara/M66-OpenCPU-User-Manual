...
/*
 The function APP_DEBUG is used as a custom defined printf() statement that outputs data through its debug port(UART_PORT1).
  The output of this function can be seen in the debug section of the QFLASH Tool 
*/
...
static void GPRS_TCP_Program(void)
{
    ...
    //7. Send data to socket
    {
        ...
        // The following will send "A B C D E F G" to the server. Change this if you want send some other string.
        // Any data format you would like to send has to be sent as a u8 data type daraLen needs to hold its length in bytes.  
...
/*
    ...
    //The functions given below are used to manually close the socket and the GPRS. You can invoke them based on any condition in the main function
    //8. Close socket
...
static void Callback_Socket_Read(s32 socketId, s32 errCode, void* customParam )
{
    ...
    do
    {
        //The function below will receive the socket data and place it in m_SocketRcvBuf
