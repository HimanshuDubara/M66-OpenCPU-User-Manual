...
static u32 m_nInterval = 120000; // 12000ms ie 2 minutes
...
void SMS_TextMode_Send(void)
{
    s32 iResult;
    u32 nMsgRef;
    char strPhNum[] = "+918897520530\0";
    char strTextMsg1[] = "NETLIGHT STATUS = HIGH\0";
    char strTextMsg2[] = "NETLIGHT STATUS = LOW\0";
...
*/
    s32 pinLevel = Ql_GPIO_GetLevel(gpioPin);
    if(pinLevel)
    {
        iResult = RIL_SMS_SendSMS_Text(strPhNum, Ql_strlen(strPhNum), LIB_SMS_CHARSET_GSM, strTextMsg1, Ql_strlen(strTextMsg1), &nMsgRef);
        if (iResult != RIL_AT_SUCCESS)
        {   
            APP_DEBUG("< Fail to send Text SMS, iResult=%d, cause:%d >\r\n", iResult, Ql_RIL_AT_GetErrCode());
            return;
        }
        APP_DEBUG("< Send Text SMS(HIGH) successfully, MsgRef:%u >\r\n", nMsgRef);
    }
    else
    {
        iResult = RIL_SMS_SendSMS_Text(strPhNum, Ql_strlen(strPhNum), LIB_SMS_CHARSET_GSM, strTextMsg2, Ql_strlen(strTextMsg2), &nMsgRef);
        if (iResult != RIL_AT_SUCCESS)
        {   
            APP_DEBUG("< Fail to send Text SMS, iResult=%d, cause:%d >\r\n", iResult, Ql_RIL_AT_GetErrCode());
            return;
...
static void Hdlr_RecvNewSMS(u32 nIndex, bool bAutoReply)
{
    ...
    if(TRUE == pDeliverTextInfo->conPres)  //Receive CON-SMS segment
    {
        ...
        //Toggling the pin as per the SMS data
        
        if(Ql_strcmp(on,g_asConSMSBuf[iBufIdx].asSeg[uSeg-1].aData)==0)
        {
            Ql_GPIO_SetLevel(gpioPin, PINLEVEL_HIGH);
            APP_DEBUG("Pin set to HIGH");
        }
        else
        {
            if(Ql_strcmp(off,g_asConSMSBuf[iBufIdx].asSeg[uSeg-1].aData)==0)
            {
                Ql_GPIO_SetLevel(gpioPin, PINLEVEL_LOW);
                APP_DEBUG("Pin set to LOW");
            }
            else
            {
                APP_DEBUG("Wrong input");   
            }
    ...
    APP_DEBUG("data = %s\r\n",(pDeliverTextInfo->data));
        ...
        if(Ql_strcmp(on,pDeliverTextInfo->data)==0)
        {
            Ql_GPIO_SetLevel(gpioPin, PINLEVEL_HIGH);
            APP_DEBUG("Pin set to HIGH");
        }
        ...
        {
            if(Ql_strcmp(off,pDeliverTextInfo->data)==0)
            {
                Ql_GPIO_SetLevel(gpioPin, PINLEVEL_LOW);
                APP_DEBUG("Pin set to LOW");
            }
